...
await import('../../src/core/translate/pipeline.core.js');
const core = (globalThis as any).PaperParallelPipeline as {
  runTranslationPipeline: (blocks: any[], opts: any) => Promise<any>;
  extractTerms: (text: string) => any[];
...
describe('translate pipeline core(与浏览器探针同一份源码)', () => {
  it('两遍法 + 术语抽取 + 串行保序 + 重试校验 + 失败续跑', async () => {
    const res = await core.runTranslationPipeline(blocks, {
    ...
    expect(res.blocks.find((b: any) => b.id === 'b03')).toMatchObject({ status: 'done', attempts: 2 });
    expect(res.blocks.find((b: any) => b.id === 'b04').attempts).toBe(2);
    expect(res.blocks.find((b: any) => b.id === 'b05').status).toBe('failed');
    expect(res.blocks.find((b: any) => b.id === 'b06').status).toBe('done');
  ...
  it('extractTerms:捕获带缩写、无缩写术语,去重', () => {
    const terms = core.extractTerms('零知识虚拟机（Zero-Knowledge Virtual Machine, zkVM）和证明（Proof）。以及零知识虚拟机（Zero-Knowledge Virtual Machine, zkVM）');
  ...
  it('validateTranslation:拒绝说明前缀与近全英文', () => {
    expect(core.validateTranslation('翻译说明:这是译文', 'some english source text long enough').ok).toBe(false);
    expect(core.validateTranslation('This is entirely English output text', 'The source paragraph has more than twenty characters').ok).toBe(false);
    expect(core.validateTranslation('这是正常的中文译文。', 'The source paragraph has more than twenty characters').ok).toBe(true);
  });
});
