...
| R4 英文保持原样，中文统一单栏(源为单/双/混合均可页数延伸) | PDF.js 几何 + Vision 页面计划/受限补丁 + Typst 单栏 | 真实论文逐页截图 | parser / visionPagePlan / visionCorrection / visionReconcile / typstProject / browser |
...
| R8 图/表/公式原样保留 | Vision Exp 候选 + 本地几何门 + 跨页关系门 + 原始裁图 | 最终逐页 Vision/人工复核 | assetGeometryGate / visionReconcile / crossPageRelations / assets |
...
补充硬门禁：翻译前由 `structureInvariants` 检查 unit/region/asset/caption 所有权；每个译文响应由 `markerInvariants` 检查具体 marker；失败页通过 `invalidationGraph` 精确失效缓存。三者均为确定性本地错误，不允许调用 Exp 掩盖。
